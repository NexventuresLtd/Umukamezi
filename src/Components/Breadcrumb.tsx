...
const Breadcrumb = ({ items }: BreadcrumbProps) => {
  return (
    <nav className="flex items-center text-sm mb-6 bg-gradient-to-r from-gray-50 to-yellow-50 px-4 py-3 rounded-lg border border-gray-200 shadow-sm">
        ...
        {items.map((item, index) => (
          <div key={index} className="flex items-center">
            ...
            {item.path ? (
              ...
              <span className="text-gray-900 font-semibold bg-gradient-to-r from-yellow-600 to-yellow-700 bg-clip-text  px-2 py-1 rounded-md bg-white shadow-sm border border-yellow-200">
                {item.name}
              </span>
            )}
          </div>
